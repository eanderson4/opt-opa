...
#branch.txt
sqlite3 rts96.db "create table branch (num INTEGER, name VARCHAR(20), fbus INTEGER, tbus INTEGER, l DOUBLE, permlam DOUBLE, duration DOUBLE, translam DOUBLE, br_r DOUBLE, br_x DOUBLE, br_b DOUBLE, rate_a DOUBLE, rate_b DOUBLE, rate_C DOUBLE, tap DOUBLE);"
cat branch.txt | head -n -2 | sed -e '1,17d' | nl -n ln -s ' ' | sed 's/ \+/,/g' > tmpbranch
...
#gencost
sqlite3 rts96.db "create table gencost (num INTEGER, gid VARCHAR(20), size DOUBLE, type VARCHAR(20), ramp DOUBLE, c2 DOUBLE, c1 DOUBLE);"
cat gencost.txt | head -n -0 | sed -e '1,3d'  | sed 's/\t//g' | nl -n ln -s ' ' | sed 's/ \+/,/g' > tmpcost
...
#genparm.txt
sqlite3 rts96.db "create table genparm (num INTEGER, bus INTEGER, gid VARCHAR(20), id INTEGER, pg DOUBLE, qg DOUBLE, qmax DOUBLE, qmin DOUBLE, vs DOUBLE);"
cat genparm.txt | head -n -4 | sed -e '1,5d' | nl -n ln -s ' ' |  sed 's/ \+/,/g' >tmpparm
